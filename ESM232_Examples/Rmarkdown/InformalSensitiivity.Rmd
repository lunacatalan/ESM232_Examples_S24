...
eff = rnorm(mean=0.6, sd = 0.1, n=20) # get a bunch of values for efficiencty 
site2 = eff %>% 
  map(~solarpv( area=0.1, solar=sierraczosolar, # use map to run all eff values in model
                clr="green", eunit="W", g=FALSE, etype="direct", 
                eff=.x )) # add eff values in model

# list of different outputs based on all eff values 
head(site2)
